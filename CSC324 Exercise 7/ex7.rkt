...
(define (continuations expr)
  ...
  (if (list? expr)
      ...
      ; starting point.
      ;(let* ([accum (accumulate-continuations expr)]
      ;      [fn (first accum)]
      ;      [rn (rest accum)])
      ;  (append fn (list (list (car rn) '_)))
      ;  )

      (let* ([ex expr][accum (accumulate-continuations expr)] [fn (first accum)])
        (append fn (list(list ex '_)))
...
(define (continuations-for-position i acc)
  (let* (; The current continuation pairs and expression datum
         ...
         ;[pairs-i (append current-pairs (if (list? subexpr-i) (continuations subexpr-i) (evaluate subexpr-i)))]
         [test-i (append current-pairs (if (list? subexpr-i)
         (pair-continuations (continuations subexpr-i) (second acc) i '()) (list (list subexpr-i (substitute-i '_)))))]
         [curr-expr (if (list? subexpr-i)
                        (if (equal? i (- (length current-expr) 1)) current-expr (list-set current-expr i (evaluate subexpr-i))) current-expr)]
         )
    ; Return a new list containing the updated accumulator (remember that
    ; you're returning two values here).
    (list test-i curr-expr)))
...
(define (pair-continuations subexpr expr i accum)
 (if (null? subexpr) accum
 (let ([fn (first subexpr)] [rn (rest subexpr)])
 (if (list? (first fn)) (pair-continuations rn expr i (append accum (list (list (first fn) (list-set expr i '_)))))
 (pair-continuations rn expr i (append accum (list (list (first fn) (list-set expr i (second fn)))))))
   )))

;(pair-continuations '((+ ('_ 2 3)) (2 (+ '_ 3)) (3 (+ 2 '_))) '(+ (+ 2 3) (+ 8 9)) 1 '())
